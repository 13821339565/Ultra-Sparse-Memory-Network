...
# 以下为可修改的几个配置
export run_name="1b_7b_mem_test"
CONFIG_PATH=${CODE_DIR}/configs/exps/OLMoE-1B-7B-0906_reproduce.yml

SAVE_DIR="/mnt/hdfs/hzh_ssd_hldy/olmo_exp/${run_name}"
# =========================== 重要配置 ===========================
...
if [ -e "${OLMO_ENV_INIT_FLAG_FILE}" ]; then
    ...
    if [ -d /opt/tiger/apex ]; then
        echo "/opt/tiger/apex exists, skip ..."
    else
        pip3 install --no-cache-dir setuptools==69.5.1 && export TORCH_CUDA_ARCH_LIST="8.0 9.0" && \
        cd /opt/tiger && git clone git@code.byted.org:pytorch/apex.git && cd apex && \
        git reset 7b73b12361068a10b0f44844534613f252a5ea75 && \
        cp /opt/tiger/olmoe/apex_setup.py setup.py && \
...
if [ -e ${SAVE_DIR}/latest_checkpointed_iteration.txt ]; then
    ...
    if [ -d ${CODE_DIR}/${CUR_STEP} ]; then
        ...
        cp -r /mnt/hdfs/hzh_ssd_hldy/olmo_exp/${run_name}/${CUR_STEP} ${CODE_DIR}/
    fi
